...
class FourierCirclesScene(Scene):
    ...
    parametric_function_step_size: float = 0.001
    drawn_path_color: str = YELLOW
    drawn_path_stroke_width: int = 2
    interpolate_config: list = [0, 1]
    
    # Zoom config
    include_zoom_camera: bool = False
    scale_zoom_camera_to_full_screen: bool = False
    scale_zoom_camera_to_full_screen_at: int = 4
    zoom_factor: float = 0.3
    zoomed_display_height: int = 3
    zoomed_display_width: int = 4
    image_frame_stroke_width: int = 1
    
    zoomed_camera_config: dict = {
        "default_frame_stroke_width": 3,
        "cairo_line_width_multiple": 0.05,
    }
    
    zoom_position: callable = lambda self, mob: mob.move_to(ORIGIN)
    
    zoom_camera_to_full_screen_config: dict = {
        "run_time": 3,
        "func": there_and_back_with_pause,
        "velocity_factor": 1,
    }
    
    wait_before_start: float | None = None
...
class AbstractFourierOfTexSymbol(FourierCirclesScene):
    ...
    run_time: int = 10
    tex: str = r"\rm M"
    start_drawn: bool = True
    add_path: bool = False
    path_custom_position: callable = lambda self, mob: mob
    max_circle_stroke_width: int = 1
    tex_class: type = Tex
    ...
    include_zoom_camera: bool = False
    scale_zoom_camera_to_full_screen: bool = False
    scale_zoom_camera_to_full_screen_at: int = 1
    zoom_position: callable = lambda self, mob: mob.scale(0.8).move_to(ORIGIN).to_edge(RIGHT)
...
class FourierFromSVG(AbstractFourierFromSVG):
    start_drawn: bool = True
    file_name: Optional[str] = 'D:\\InspireTale\\ManimProject\\manim-sample\\yuika.svg'
    svg_config: dict = {
        "fill_opacity": 0,
        "stroke_color": WHITE,
        "stroke_width": 1,
    ...
    drawn_path_color: str = YELLOW
    interpolate_config: List[float] = [0, 1]
    n_vectors: int = 50
    big_radius: float = 2
    drawn_path_stroke_width: float = 2
    center_point: any = ORIGIN
